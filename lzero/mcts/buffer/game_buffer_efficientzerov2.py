...
from lzero.mcts.tree_search.mcts_ctree import EZV2MCTSCtree as EZEV2MCTSCtree
...
@BUFFER_REGISTRY.register('game_buffer_efficientzerov2')
class EfficientZeroV2GameBuffer(MuZeroGameBuffer):
    """
    Overview:
    ...
    def sample(self, batch_size: int, policy: Any) -> List[Any]:
        ...
        # target value_prefixs, bootstrap values, search values
        batch_value_prefixs, batch_target_values, batch_search_values = self._compute_target_reward_value(
        ...
        # EfficientZero V2: add search_values to target_batch for value mixing
        target_batch = [batch_value_prefixs, batch_target_values, batch_target_policies, batch_search_values]
    ...
    def _prepare_reward_value_context(
        ...
        # EfficientZero V2: collect search values from game segments
        search_values_list = []
        ...
        for game_segment, state_index, idx in zip(game_segment_list, pos_in_game_segment_list, batch_index_list):
            ...
            # EfficientZero V2: collect MCTS search values from collector
            search_values_list.append(game_segment.root_value_segment)
        ...
        reward_value_context = [
            ...
            action_mask_segment, to_play_segment, search_values_list
        ]
    ...
    def _compute_target_reward_value(self, reward_value_context: List[Any], model: Any) -> List[np.ndarray]:
        ...
        to_play_segment, search_values_list = reward_value_context  # noqa
        ...
        # ==============================================================
        # EfficientZero V2 related core code
        # ==============================================================
        batch_target_values, batch_value_prefixs = [], []
        batch_search_values = []  # EfficientZero V2: collect search values for mixing
        with torch.no_grad():
            ...
            for game_segment_len_non_re, reward_list, state_index, to_play_list, search_value_segment in zip(
                game_segment_lens, rewards_list, pos_in_game_segment_list, to_play_segment, search_values_list
            ):
                target_values = []  # bootstrap values
                target_value_prefixs = []
                search_values = []  # search values from collector
                value_prefix = 0.0
                ...
                for current_index in range(state_index, state_index + self._cfg.num_unroll_steps + 1):
                    ...
                    if current_index < game_segment_len_non_re:
                        # Bootstrap value (n-step TD)
                        target_values.append(value_list[value_index])
                        # Search value (from MCTS during collection)
                        search_values.append(search_value_segment[current_index])
                ...
                batch_search_values.append(search_values)
        ...
        batch_search_values = np.asarray(batch_search_values, dtype=object)

        return batch_value_prefixs, batch_target_values, batch_search_values
    ...
    def _compute_target_policy_reanalyzed(self, policy_re_context: List[Any], model: Any) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            value_prefix_pool = value_prefix_pool.squeeze().tolist()
            policy_logits_pool = policy_logits_pool.tolist()
            # EfficientZero V2: use zero Gumbel noises for reanalyze (no exploration)
            gumbel_noises = [
                np.zeros(len(legal_actions[i]), dtype=np.float32) for i in range(transition_batch_size)
            ]

            if self._cfg.mcts_ctree:
                # EfficientZero V2: cpp mcts_tree
                roots = EZEV2MCTSCtree.roots(transition_batch_size, legal_actions)
                roots.prepare_no_noise(value_prefix_pool, policy_logits_pool, to_play)
                # do MCTS for a new policy with the recent target model
                improved_policies, best_actions = EZEV2MCTSCtree(self._cfg).search(
                    roots, model, latent_state_roots, reward_hidden_state_roots, to_play, gumbel_noises
                )
            else:
                # python mcts_tree (not implemented for V2 yet, fallback to V1)
                roots = MCTSPtree.roots(transition_batch_size, legal_actions)
                roots.prepare_no_noise(value_prefix_pool, policy_logits_pool, to_play)
                MCTSPtree(self._cfg).search(
                    roots, model, latent_state_roots, reward_hidden_state_roots, to_play=to_play
                )
                # Fallback: use visit count distributions for python tree
                improved_policies = roots.get_distributions()
                best_actions = [np.argmax(d) for d in improved_policies]
